...
(function(){
  ...
  function readerData() {
    ...
    var getFictionInfo = function(callback) {
      $.get('chapter',function(data){
        ...
        //本应当在此处处理章节id和章节数等信息，由于目前数据少且固定，暂时省略
      ...
      $.get('/chapter_data',{id:chapterId},function(data){
        ...
        if(data.result == 0) {
          ...
          utl.getBSONP(url,function(data) {
            ...
            callback && callback(data); //这里的回调将访问url获取的内容渲染到页面
          })
        }
      },'json')
    }
  ...
  function readerFrame(container) {
    //初始化UI结构，它的返回值是一个函数，该函数传入要渲染的数据，并将其渲染到container中
  ...
  function eventHandler() {
    ...
    $('.menu').click(function(event){
      ...
      //要确定Index是有效值，因为点击menu也会触发事件，index为undefined
      if(index > 0){
    ...
    $('.bk-container').click(function(){
      ...
      dom.page.css('background-color',backgroundColor);
      //选中提示框随之变化(先隐藏全部，然后使触发事件的那个框的子元素显示)
      dom.currnetBk.hide();
      $this.children().show();
